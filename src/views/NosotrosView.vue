...
<template>
  ...
  <main data-aos>
    <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24">
      <div class="flex justify-center">
        <div class="max-w-3xl text-center text-slate-800 dark:text-slate-300 text-lg leading-relaxed font-glacial">
  ...
  <BaseModal :show="selectedItem !== null" @close="selectedItem = null">
    <div v-if="selectedItem" class="text-center">
      <h3 class="text-2xl font-prompt uppercase text-primary mb-4">{{ selectedItem.titulo }}</h3>
      <img :src="selectedItem.imagen" :alt="selectedItem.titulo" class="rounded-lg mb-4 w-full" loading="lazy">
      <p class="text-slate-800 dark:text-slate-300 text-lg font-glacial">{{ selectedItem.descripcion }}</p>
    </div>
  </BaseModal>
</template>
